...
class S3LoaderVows(Vows.Context):
  ...
  class CanDetectBucket(Vows.Context):
    def topic(self):
      return presigning_loader._get_bucket('/'.join([s3_bucket, IMAGE_PATH]))
  ...
  class CanNormalize(Vows.Context):
    def topic(self):
      return presigning_loader._normalize_url('/'.join([s3_bucket, IMAGE_PATH]))
  ...
  class CanBuildPresignedUrl(Vows.Context):
    ...
    def should_generate_presigned_urls(self, topic):
      print(presigning_loader)
      url = presigning_loader._generate_presigned_url(topic)
